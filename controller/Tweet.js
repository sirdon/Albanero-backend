...
module.exports = {
    ...
    getOthersAllTweet(req, res) {
        try {
            Tweets.find({ userId: { $ne: req.user._id }, isActive: true }).exec((err, tweets) => {
                if (err || !tweets) {
                    return res.status(400).json({
                        message: "Tweets not found", status: false
                    });
        ...
        try {
            ...
            Tweets.findOne({ _id: tweetId, isActive: true }).exec((err, tweet) => {
                if (err || !tweet) {
                    ...
                    } else if (tweet.userId == req.user._id) {
                        return res.status(400).send({
                            status: false,
                            message: "You cannot like/unlike your tweet"
                        })
                    } else {
                        let index = tweet.likeUserId.findIndex(id => id == req.user._id);
                        if (index == -1) { 
                            tweet.likeUserId.push(req.user._id)
                            tweet.like = tweet.like + 1;
                            emailServise.sendLikeEmail(tweet.userId, req.user._id);
                        }else if (index > -1) {
                            tweet.like = tweet.like - 1;
                            tweet.likeUserId.splice(index, 1);
                        }
                    } 
                    tweet.save()
